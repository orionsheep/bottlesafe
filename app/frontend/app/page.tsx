    ...
    <main className={lang === "zh" ? "lang-zh" : ""}>
      <section className={`hero ${mode}`} id="top" onMouseMove={moveHouse} onMouseLeave={() => setTilt({ x: 0, y: 0 })} style={heroStyle}>
        <nav className="nav" aria-label="Main navigation">
          ...
          <div className="nav-links"><a href="#index">{t.navIndex}</a><a href="#method">{t.navMethod}</a><a href="#action">{t.navAction}</a><a href="/scan">{t.navScan}</a><a href="/archive">{lang === "zh" ? "档案工作台" : "Archive desk"}</a></div>
