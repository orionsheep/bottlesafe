...
def load_model(model_id: str, adapter: str | None):
    kwargs = dict(device_map="auto", torch_dtype=torch.bfloat16, trust_remote_code=True)
    if torch.cuda.is_available():
        # 仅 CUDA 环境使用 4-bit 量化；macOS (MPS) 等无 CUDA 环境以 bf16 全量加载。
...
def analyze_image(model, processor, image_path: str, return_raw: bool = False) -> ChemicalAnalysis | str:
    ...
    if return_raw:
        return answer
    return ChemicalAnalysis.model_validate(extract_json(answer))
