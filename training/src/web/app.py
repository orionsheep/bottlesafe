...
环境变量：
    CHEM_MODEL_PATH  基础模型路径（默认 ../04_基础模型/Qwen3-VL-4B-Instruct）
    CHEM_ADAPTER     LoRA 适配器路径（可选，默认不加载）
...
BASE_DIR = Path(__file__).resolve().parents[2]          # 01_项目代码
PROJECT_DIR = BASE_DIR.parent                            # 家庭化学品安全模型
WEB_DIR = BASE_DIR / "web"
UPLOAD_DIR = BASE_DIR / "data" / "uploads"
UPLOAD_DIR.mkdir(parents=True, exist_ok=True)

MODEL_PATH = os.environ.get("CHEM_MODEL_PATH", str(PROJECT_DIR / "04_基础模型" / "Qwen3-VL-4B-Instruct"))
...
def analyze(image: UploadFile = File(...)):
    ...
    with _infer_lock:
        try:
            analysis = analyze_image(model, processor, str(dest))
